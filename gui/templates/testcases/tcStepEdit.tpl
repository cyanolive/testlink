  ...
  <table class="simple">
	{if $gui->steps_results_layout == "horizontal"}
      ...
      {if $session['testprojectOptions']->automationEnabled}
  		  <th style="display:none;" width="25">{$labels.execution_type_short_descr}</th>
  		{/if}  
  	</tr>
  ...
  {if $gui->tcaseSteps != ''}
    ...
    {assign var=row value=0}
      ...
      {else}
        ...
        {if $session['testprojectOptions']->automationEnabled}
  	  	  <td style="display:none;"><a href="{$hrefEditStep}{$step_info.id}">{$gui->execution_types[$step_info.execution_type]}</a></td>
  	  	{/if}  
      {/if}
		{* BUGID 3583 - Amit - horizintal row between test case steps *}
  ...
  {else} {* Vertical layout *}
		{foreach from=$gui->tcaseSteps item=step_info}
			<tr id="step_row_{$step_info.step_number}">
				...
				{if $session['testprojectOptions']->automationEnabled}
					{if $step_info.step_number == $gui->step_number}
					<th style="display:none;" width="200">{$labels.execution_type_short_descr}:
						<select name="exec_type" onchange="content_modified = true">
							{html_options options=$gui->execution_types selected=$gui->step_exec_type}
			  	      </select>
					</th>
					{else}
						<th style="display:none;">{$labels.execution_type_short_descr}:
