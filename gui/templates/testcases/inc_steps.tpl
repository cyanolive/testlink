...
{if $layout == 'horizontal'}
	<tr>
		...
		<th style="display:none;" width="25">{$inc_steps_labels.execution_type_short_descr}</th>
		{/if}
		{if $edit_enabled}
		<th>&nbsp;</th>
		<th>&nbsp;</th>
	...
	<tr id="step_row_{$step_info.step_number}">
		...
		{if $session['testprojectOptions']->automationEnabled}
		<td style="display:none;" {if $edit_enabled} style="cursor:pointer;" onclick="launchEditStep({$step_info.id})" {/if}>{$gui->execution_types[$step_info.execution_type]}</td>
		{/if}
...
{else}
	...
	<tr>
		...
		<th>{$inc_steps_labels.step_actions}</th>
		{if $session['testprojectOptions']->automationEnabled}
		<th style="display:none;">{$inc_steps_labels.execution_type_short_descr}:
